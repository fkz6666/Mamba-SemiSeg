...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if C.dim() == 3:
            ...
            ctx.squeeze_C = True
        out, x, *rest = selective_scan_cuda.selective_scan_fwd(u, delta, A, B, C, D, z, delta_bias, delta_softplus)
...
def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    """
    out = SelectiveScanFn.apply(u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state)

    if out is None:
        # Handle the error case appropriately - either raise an informative error
        # or return a zero tensor with appropriate shape
        batch, dim, dstate = u.shape[0], A.shape[0], A.shape[1]
        if return_last_state:
            out = (torch.zeros_like(u), torch.zeros(batch, dim, dstate, device=u.device))
        else:
