...
from mamba_ssm.utils.torch_utils import custom_bwd, custom_fwd
...
def selective_scan_bwd(dout, x, dt, A, B, C, D=None, z=None):
    ...
    from selective_scan_cuda import selective_scan_fwd,  selective_scan_bwd
    ...
    if z is not None and z.stride(-1) != 1:
        z = z.contiguous()
    _, intermediate, *rest = selective_scan_fwd(x, dt.to(dtype=x.dtype), A, B, C, D, z, None, False)
    if z is not None:
        out = rest[0]
    else:
        out = None
    ...
    _, ddt, dA, *rest = selective_scan_bwd(
